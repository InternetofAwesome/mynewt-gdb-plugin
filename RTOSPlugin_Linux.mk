...
#
# 		make -f RTOSPlugin_Linux.mk TARGET_NAME=i386 release 	# builds only the release version
# 		make -f RTOSPlugin_Linux.mk TARGET_NAME=i386 debug 		# builds only the debug version
# 		make -f RTOSPlugin_Linux.mk TARGET_NAME=i386 all 	 	 	# builds release and debug versions
# 		make -f RTOSPlugin_Linux.mk TARGET_NAME=i386 clean 	 	# removes all the generated binaries
#
# 64-bit Linux targets
# ====================
# Invoke 'make' like this:
#
# 		make -f RTOSPlugin_Linux.mk TARGET_NAME=x86_64 release 	# builds only the release version
# 		make -f RTOSPlugin_Linux.mk TARGET_NAME=x86_64 debug 		# builds only the debug version
# 		make -f RTOSPlugin_Linux.mk TARGET_NAME=x86_64 all 	 	 	# builds release and debug versions
# 		make -f RTOSPlugin_Linux.mk TARGET_NAME=x86_64 clean 	 	# removes all the generated binaries
#
...
#
# Set a default value for the target name if not specified.
#
ifeq ($(TARGET_NAME),)
  TARGET_NAME = x86_64
endif
...
$(RELEASE_SHARED): $(addprefix $(RELEASE_DIR)/, $(OBJECTS))
	$(V)$(ECHO) "LIB $@"
	$(V)$(LINK.o) $(LDFLAGS) -fPIC -shared -Wl,--strip-all -Wl,-soname,$(RELEASE_LIB_SONAME) $^ -o $@ $(addprefix -l,$(LIBS))

$(DEBUG_SHARED): $(addprefix $(DEBUG_DIR)/, $(OBJECTS))
	$(V)$(ECHO) "LIB $@"
	$(V)$(LINK.o) $(LDFLAGS) -fPIC -shared -Wl,-soname,$(DEBUG_LIB_SONAME) $^ -o $@ $(addprefix -l,$(LIBS))
